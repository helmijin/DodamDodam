...
function joinSendit() {
	...
	if(form.userid.value=="") {
		...
		alert("생년월일을 입력해 주세요.");
		form.b_day.focus();
	} else if(form.ph1.value=="") {
		alert("휴대폰번호를 입력해 주세요.");
		form.hp1.focus();
	} else if(form.ph2.value=="") {
		alert("휴대폰번호를 입력해 주세요.");
		form.hp2.focus();
	} else if(form.ph3.value=="") {
		alert("휴대폰번호를 입력해 주세요.");
		form.hp3.focus();
...
function joinSendit2(gubun) {
	...
	if(gubun==3){ // 반려견 추가 클릭시
		...
		if(choose) {	form.gubun.value=1; form.submit(); }
		else { return; }
...
function id_search1(){
	...
	if(form.name.value=="") {
		...
		$.ajax({
				url:'/bodeum/edu/searchid',
				type:'GET',
				data:'name='+encodeURI($('#id_name').val())+'&email1='+encodeURI($('#id_email1').val())+'&email2='+encodeURI($('#id_email2').val()),
				success:function(userid){
				alert('회원님의 아이디는 : '+userid+'입니다');
					$('#search_id').html(userid);
				},
				error:function(){
				alert('error');
				}
...
function id_search2(){
	...
	if(form.name.value=="") {
		...
		$.ajax({
				url:'/bodeum/edu/searchid2',
				type:'GET',
				data:'name='+encodeURI($('#name').val())+'&ph1='+encodeURI($('#ph1').val())+'&ph2='+encodeURI($('#ph2').val())+'&ph3='+encodeURI($('#ph3').val()),
				success:function(userid){
					alert('회원님의 아이디는 : '+userid+'입니다');
...
function dogDel(idx){
	...
	if(choose) {	location.href='dog_del_ok.php?idx='+idx; }
	else { return; }
}
...
function actSelect()
{
	...
	if(typeof(f.cart_list) == 'object') {
		if(f.cart_list.length) {
			for (i=0,j=0;i<f.cart_list.length;i++) { if(f.cart_list[i].checked) { arr_cart_list[i] = f.cart_list[i].value;j++; }}
			if(!j) { alert('상품을 선택하여 주세요.');return; }
			else f.arr_cart_list.value = arr_cart_list.join('@');
		} else {
			if(!f.cart_list.checked) { alert('상품을 선택하여 주세요.');return; }
		}
		if(j==1) f.arr_cart_list.value = '';
		if(confirm('선택한 상품을 삭제하시겠습니까?')) f.submit();
	} else {
		alert('선택할 상품이 없습니다.');	return;
	}
}
...
function actSelect2() {
	...
	if(typeof(fm.cart_list) == 'object') {
		if(fm.cart_list.length) {
			for (i=0,j=0;i<fm.cart_list.length;i++) { if(fm.cart_list[i].checked) { arr_cart_list[i] = fm.cart_list[i].value;j++; }}
			if(!j) { alert('상품을 선택하여 주세요.');return; }
			else fm.arr_cart_list.value = arr_cart_list.join('@');
		} else {
			if(!fm.cart_list.checked) { alert('상품을 선택하여 주세요.');return; }
		}
...
function actSelect3()
{
	...
	if(typeof(f.del_list) == 'object') {
		if(f.del_list.length) {
			...
			if(!j) { alert('상품을 선택해주세요.');return; }
			else f.arr_del_list.value = arr_del_list.join('@');
		} else {
			if(!f.del_list.checked) { alert('상품을 선택해주세요.');return; }
		}
	...
	if($("input:checkbox[id='checkOne']").is(":checked") == true) {
		...
		$.ajax({
			   url:'mem_addr_update.php',
				type:'POST',
				data: {"postcode":postcode,"addr1":addr1,"addr2":addr2,"userid":userid},
				success:function(data){
					$('#addr_info').html(data);
				}
		});
...
function alertGoLecture(){
	...
	if(choose) {	location.href='../../edu_movie/teach/edu_request_list.php'; }
	else { return; }
}
